...
mdxnet-infer separates drum audio into individual component stems using the MDX23C TFC-TDF architecture. It ships one community-trained DrumSep model (6-stem), trained by aufr33 and jarredou. Model weights are downloaded automatically on first use, sha256-verified against org-recorded digests (see [Weights provenance](#weights-provenance) below).
...
| `drumsep-5stem` | kick, snare, toms, hh, cymbals | **Unavailable** — lost upstream, see [Weights provenance](#weights-provenance) |
